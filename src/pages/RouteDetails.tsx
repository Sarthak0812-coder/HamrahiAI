...
import { ArrowLeft, Clock, Ticket, Eye } from "lucide-react";
    ...
    <div className="min-h-screen pb-8 px-4 pt-4">
      ...
      <div className="relative ml-4 border-l-2 border-primary/30 pl-6 space-y-6 mb-8">
        {route.stops.map((stop, index) => (
          <div key={`${stop}-${index}`} className="relative animate-float-up" style={{ animationDelay: `${index * 60}ms` }}>
            <div
              className={`absolute -left-[31px] w-4 h-4 rounded-full border-2 ${
                index === 0 || index === route.stops.length - 1 ? "bg-primary border-primary" : "bg-card border-primary/50"
              }`}
            />
            <p className={`text-sm ${index === 0 || index === route.stops.length - 1 ? "font-bold text-foreground" : "text-muted-foreground"}`}>
              {stop}
            </p>
            {index === 0 && <p className="text-xs text-primary mt-0.5">Boarding</p>}
            {index === route.stops.length - 1 && <p className="text-xs text-crowd-low mt-0.5">Destination</p>}
          </div>
        ))}
      </div>
