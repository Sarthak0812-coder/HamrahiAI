...
import { TrainFront, Waypoints, CreditCard, Check, Ticket, QrCode, X, Plus } from "lucide-react";
import { useLocation } from "react-router-dom";
import { QRCodeSVG } from "qrcode.react";
import { stations, type BookedTicket, type Route } from "../data/mockData";

type Tab = "book" | "tickets";

const TicketingPage = () => {
  const location = useLocation();
  const routeState = (location.state as { route?: Route } | null)?.route;
  ...
  const hasLocal = modes.some((mode) => mode.includes("Train"));
  const hasMetro = modes.some((mode) => mode.includes("Metro"));
  ...
  const handleBook = () => {
    ...
    const ticket: BookedTicket = {
      ...
      to,
      modes,
      price: `Rs. ${combinedPrice}`,
      date: new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }),
      time: new Date().toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" }),
      status: "active",
      qrData: JSON.stringify({ id: ticketId, from, to, modes, price: combinedPrice, ts: Date.now() }),
      passengerName,
    };

    const existing = JSON.parse(localStorage.getItem("hamrahi-tickets") || "[]") as BookedTicket[];
  ...
  if (booked) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4">
        ...
        <h2 className="text-xl font-bold text-foreground mb-2 animate-float-up" style={{ animationDelay: "100ms" }}>
          Ticket Booked!
        </h2>
        <p className="text-sm text-muted-foreground animate-float-up" style={{ animationDelay: "200ms" }}>
  ...
  return (
    <div className="min-h-screen pb-24 px-4 pt-6">
      ...
      <div className="flex gap-1 bg-secondary/50 rounded-2xl p-1 mb-6">
        <button
          onClick={() => setTab("book")}
          className={`flex-1 py-2.5 rounded-xl text-xs font-semibold transition-all ${tab === "book" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
        >
          Book Ticket
        </button>
        <button
          onClick={() => setTab("tickets")}
          className={`flex-1 py-2.5 rounded-xl text-xs font-semibold transition-all ${tab === "tickets" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
        >
      ...
      {tab === "book" && (
        <div className="animate-float-up">
          <div className="glass-card p-4 mb-4">
            ...
            <select value={from} onChange={(event) => setFrom(event.target.value)} className="w-full bg-secondary/50 rounded-xl px-3 py-2.5 text-sm text-foreground outline-none mb-3 appearance-none">
            ...
            <select value={to} onChange={(event) => setTo(event.target.value)} className="w-full bg-secondary/50 rounded-xl px-3 py-2.5 text-sm text-foreground outline-none appearance-none">
              <option value="">Select station</option>
              {stationNames.map((station) => (
                <option key={station} value={station}>
                  {station}
          ...
          <div className="glass-card p-4 mb-4">
            ...
            <div className="flex gap-2">
              {["Local Train", "Metro"].map((mode) => (
                <button
                  key={mode}
                  onClick={() => {
                    setModes((current) => (current.includes(mode) ? (current.length > 1 ? current.filter((item) => item !== mode) : current) : [...current, mode]));
                  }}
                  className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-semibold transition-all ${
                    modes.includes(mode) ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground"
                  }`}
                >
                  {mode.includes("Train") ? <TrainFront className="w-3.5 h-3.5" /> : <Waypoints className="w-3.5 h-3.5" />}
                  {mode}
                </button>
              ))}
            </div>
            {isCombined && (
              <div className="bg-primary/10 rounded-xl px-3 py-2 flex items-center gap-2 mt-3">
                ...
                <span className="text-xs text-primary font-semibold">Combined ticket - Save Rs. {savings}!</span>
              </div>
            )}
          </div>
          ...
          <div className="glass-card p-4 mb-5">
            ...
            {hasLocal && (
              <div className="flex justify-between mb-2">
                ...
                <span className="text-sm text-foreground">Rs. {localPrice}</span>
              </div>
            )}
            {hasMetro && (
              <div className="flex justify-between mb-2">
                <span className="text-sm text-muted-foreground">Metro</span>
                <span className="text-sm text-foreground">Rs. {metroPrice}</span>
              </div>
            )}
            {isCombined && savings > 0 && (
              <div className="flex justify-between mb-2">
                <span className="text-sm text-crowd-low">Combined Discount</span>
                <span className="text-sm text-crowd-low font-semibold">-Rs. {savings}</span>
              </div>
            )}
            <div className="border-t border-border/50 my-3" />
            <div className="flex justify-between items-center">
              <span className="text-sm font-semibold text-foreground">Total</span>
              <span className="text-lg font-bold text-primary">Rs. {combinedPrice}</span>
            </div>
          </div>
          ...
          >
            Pay Rs. {combinedPrice} & Get QR Ticket
          </button>
        </div>
      )}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${ticket.status === "active" ? "bg-crowd-low/15 text-crowd-low" : "bg-secondary text-muted-foreground"}`}>
                          {ticket.status.toUpperCase()}
                        </span>
                      </div>
                      <p className="text-sm font-semibold text-foreground">{ticket.from} to {ticket.to}</p>
                      <p className="text-xs text-muted-foreground">{ticket.date} • {ticket.modes.join(" + ")} • {ticket.price}</p>
                    </div>
          ...
          <div className="glass-card p-6 w-full max-w-sm animate-float-up" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="space-y-2">
              {[
                ...
                ["Route", `${selectedTicket.from} to ${selectedTicket.to}`],
