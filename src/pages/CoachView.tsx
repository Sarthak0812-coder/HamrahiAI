...
const CoachView = () => {
  ...
  const filteredTrains = lineFilter === "All" ? mockTrains : mockTrains.filter((train) => train.line === lineFilter);
  ...
  const predictionStops = selectedTrain
    ? stations.filter((station) => station.line === selectedTrain.line.toLowerCase()).slice(0, 3).map((station) => station.name)
    : [];
      ...
      <div className="min-h-screen pb-24 px-4 pt-6">
          ...
          {lines.map((line) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-semibold transition-all shrink-0 ${
                lineFilter === line ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground"
              }`}
            >
              {line}
            </button>
        ...
        <div className="space-y-3">
          {filteredTrains.map((train, index) => {
            ...
            const avgCrowd = Math.round(trainCoaches.reduce((sum, item) => sum + item.crowd, 0) / trainCoaches.length);
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">{train.from} to {train.to}</p>
  ...
  return (
    <div className="min-h-screen pb-24 px-4 pt-4">
      ...
      <p className="text-sm text-muted-foreground mb-6">
        {selectedTrain.name} • {selectedTrain.from} to {selectedTrain.to}
      </p>
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-2 overflow-x-auto pb-3 -mx-4 px-4 snap-x">
          {coaches.map((item) => {
            const isRecommended = item.id === bestCoach?.id;
            const isSelected = item.id === selectedCoach;

            return (
              <button
                key={item.id}
                onClick={() => setSelectedCoach(item.id === selectedCoach ? null : item.id)}
                className={`shrink-0 snap-center w-20 rounded-2xl border-2 p-3 flex flex-col items-center gap-1.5 transition-all duration-200 active:scale-95
                  ${isSelected ? `${crowdBorder(item.status)} bg-secondary` : "border-border bg-card"}
                  ${item.status === "high" ? "animate-pulse-crowd" : ""}
                `}
              >
                <div className={`w-8 h-8 rounded-lg ${crowdColor(item.status)} flex items-center justify-center`}>
                  <span className="text-xs font-bold text-primary-foreground">{item.crowd}%</span>
                </div>
                <span className={`text-xs font-semibold ${isRecommended ? "text-primary" : "text-foreground"}`}>{item.id}</span>
                <span className={`text-[10px] ${crowdText(item.status)}`}>{item.label}</span>
        ...
        <div className="glass-card p-4 animate-float-up">
          ...
          <div className="space-y-3">
            ...
            <div className="w-full h-2 bg-secondary rounded-full overflow-hidden">
              <div className={`h-full rounded-full transition-all duration-500 ${crowdColor(coach.status)}`} style={{ width: `${coach.crowd}%` }} />
            </div>
            <div className="pt-2 border-t border-border/50">
              <p className="text-xs text-muted-foreground mb-2">Predicted at next stations:</p>
              <div className="flex gap-2">
                {predictionStops.map((stop, index) => (
                  <div key={stop} className="glass-card px-3 py-2 text-center flex-1">
                    <p className="text-[10px] text-muted-foreground">{stop}</p>
                    <p className={`text-xs font-bold ${crowdText(coach.status)}`}>{Math.min(100, coach.crowd + (index + 1) * 8)}%</p>
                  </div>
                ))}
              </div>
            </div>
